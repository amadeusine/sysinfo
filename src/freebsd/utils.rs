...
pub fn c_buf_to_str(buf: &[u8]) -> Option<String> {
    if let Some(pos) = buf.iter().position(|x| *x == 0) {
        // Shrink buffer to terminate the null bytes
        std::str::from_utf8(&buf[..pos]).ok().map(|s| s.to_owned())
    } else {
        std::str::from_utf8(buf).ok().map(|s| s.to_owned())
    }
}

pub unsafe fn get_sys_value_str(mib: &[c_int], buf: &mut [u8]) -> Option<String> {
    ...
    c_buf_to_str(&buf[..len / mem::size_of::<u8>()])
}
...
pub fn get_system_info(value1: c_int, value2: c_int, default: Option<&str>) -> Option<String> {
