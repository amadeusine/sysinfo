...
use super::utils::{c_buf_to_str, get_sys_value_str, Wrap};
...
mod consts {
    ...
    pub const SWAIT: libc::c_char = 6;
    pub const SLOCK: libc::c_char = 7;
    pub const P_SYSTEM: libc::c_int = 0x00000200;
    pub const KERN_PROC_CWD: libc::c_int = 42;
}
...
impl From<libc::c_char> for ProcessStatus {
    fn from(status: libc::c_char) -> ProcessStatus {
        match status {
            consts::SIDL => ProcessStatus::Idle,
            consts::SRUN => ProcessStatus::Run,
            consts::SSLEEP => ProcessStatus::Sleep,
            consts::SSTOP => ProcessStatus::Stop,
            consts::SZOMB => ProcessStatus::Zombie,
            consts::SWAIT => ProcessStatus::Dead,
            consts::SLOCK => ProcessStatus::LockBlocked,
...
pub(crate) unsafe fn get_process_data(
    ...
    if kproc.ki_pid != 1 && (kproc.ki_flag as libc::c_int & consts::P_SYSTEM) != 0 {
