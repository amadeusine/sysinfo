...
use super::utils::{
    self, boot_time, c_buf_to_str, from_cstr_array, get_sys_value, get_sys_value_array,
    ...
    fn refresh_process(&mut self, pid: Pid) -> bool {
        let proc_ = {
            ...
            crate::utils::into_iter(procs)
                ...
                .find(|kproc| kproc.ki_pid == pid)
                #[cfg(feature = "multithread")]
                .find_any(|kproc| kproc.ki_pid == pid)
                .and_then(|kproc| {
        ...
        if let Some(proc_) = proc_ {
            ...
            self.process_list.get(&pid).map(|p| p.updated).unwrap_or(false)
        }
    }
...
impl System {
    unsafe fn refresh_procs(&mut self) {
        ...
        let procs = {
            ...
            crate::utils::into_iter(&mut self.process_list)
                .for_each(|_, proc_| {
                    proc_.updated = false;
                });
        ...
        for (kproc, mut proc_) in procs {
            ...
            if !proc_.cmd.is_empty() {
                ...
                if let Some(name) = p.file_name().and_then(|s| s.to_str()) {
                    ...
                    // possible.
                    let name = c_buf_to_str(std::slice::from_raw_parts(
                        kproc.ki_comm.as_ptr() as _,
                        kproc.ki_comm.len(),
                    ))
